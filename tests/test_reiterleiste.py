...
def test_relay_tabellen_nutzen_den_gemeinsamen_rollbereich():
    ...
    # Sieben Tabellen: Sende-Identitäten (v1.9), Geräte, Abgewiesen, Top-Identitäten
    # (v1.9.5), Top-Absender, Top-Empfänger (v1.8.113) und das Relay-Protokoll
    # (v1.8.114).
    assert text.count('class="tabellen-rollbereich"') == 7
    assert "overflow-x:auto" not in text, (
        "Der Rollbereich ist mit Inline-CSS nachgebaut statt die vorhandene "
        "Klasse zu nutzen.")
    assert text.count('class="config-table"') == 7, (
